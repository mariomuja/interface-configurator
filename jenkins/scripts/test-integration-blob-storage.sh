...
rm -f test-results/junit-integration-blob-*.xml
...
/usr/bin/docker run --rm \
  ...
  dotnet test tests/main.Core.Tests/main.Core.Tests.csproj \
    ...
    --logger "junit;LogFilePath=$PWD/test-results/junit-integration-blob-{assembly}.xml" \
    --results-directory "$PWD/test-results"

echo "Blob Storage integration tests completed"
