...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const url = `${functionAppUrl.replace(/\/$/, '')}/api/UpdateCsvPollingInterval`;
    ...
    if (rawPayload && rawPayload.trim().length > 0) {
      try {
        ...
        console.error('Invalid JSON body for UpdateCsvPollingInterval:', parseError);
        return res.status(400).json({ error: 'Invalid JSON body' });
      }
    }

    const interfaceName = parsedBody.InterfaceName || parsedBody.interfaceName || '';
    const pollingInterval = parsedBody.PollingInterval ?? parsedBody.pollingInterval;
    ...
    console.error('Error proxying UpdateCsvPollingInterval:', error);
    res.status(500).json({
      error: 'Proxy error',
