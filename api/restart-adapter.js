...
module.exports = async (req, res) => {
  ...
  try {
    const { interfaceName, adapterType } = req.body;

    if (!interfaceName || !adapterType) {
      res.status(400).json({ error: 'interfaceName and adapterType are required' });
      return;
    }

    if (adapterType !== 'Source' && adapterType !== 'Destination') {
      res.status(400).json({ error: 'adapterType must be "Source" or "Destination"' });
      return;
    }

    // Get function key from environment variable
    const functionKey = process.env.AZURE_FUNCTION_KEY || '';
    const functionName = 'RestartAdapter';
    ...
    res.status(500).json({
      error: 'An error occurred while restarting the adapter',
      details: error.message,
