...
Write-Host "`n3. Teste Health Check Endpoint..." -ForegroundColor Yellow
$testUrl = "https://$($functionApp.defaultHostName)/api/health"
...
try {
    ...
    Write-Host "   ✅ SUCCESS: Health check antwortet!" -ForegroundColor Green
    Write-Host "   Status Code: $($response.StatusCode)" -ForegroundColor Cyan
    $healthData = $response.Content | ConvertFrom-Json
    Write-Host "   Health Status: $($healthData.Status)" -ForegroundColor $(if ($healthData.Status -eq "Healthy") { "Green" } else { "Yellow" })
} catch {
    if ($_.Exception.Response) {
        ...
        if ($statusCode -eq 404) {
            Write-Host "   ⚠️  Health check endpoint noch nicht verfügbar" -ForegroundColor Yellow
            ...
            Write-Host "   ⚠️  Service Unavailable - Function App startet noch oder unhealthy" -ForegroundColor Yellow
